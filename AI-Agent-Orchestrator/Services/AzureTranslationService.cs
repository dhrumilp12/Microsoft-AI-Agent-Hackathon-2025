    ...
    public class AzureTranslationService
    {
        ...
        public AzureTranslationService(IConfiguration configuration, ILogger<AzureTranslationService>? logger = null)
        {
            ...
            _subscriptionKey = useEnvVars 
                ? Environment.GetEnvironmentVariable("TRANSLATOR_API_KEY") ?? Environment.GetEnvironmentVariable("AZURE_TRANSLATOR_KEY") ?? ""
                : configuration["AzureTranslator:Key"] ?? "";

            _endpoint = useEnvVars
                ? Environment.GetEnvironmentVariable("TRANSLATOR_ENDPOINT") ?? Environment.GetEnvironmentVariable("AZURE_TRANSLATOR_ENDPOINT") ?? TranslationConstants.DefaultTranslatorEndpoint
                : configuration["AzureTranslator:Endpoint"] ?? TranslationConstants.DefaultTranslatorEndpoint;

            _region = useEnvVars
                ? Environment.GetEnvironmentVariable("TRANSLATOR_REGION") ?? Environment.GetEnvironmentVariable("AZURE_TRANSLATOR_REGION") ?? ""
